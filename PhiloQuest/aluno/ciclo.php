...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao']) && $_POST['acao'] === 'enviar_etapa') {
    ...
    try {
        ...
        $mensagemErro = philoquest_user_error_message($e, 'Não foi possível processar sua solicitação.');
        $progresso = $alunoService->obterProgressoCiclo($alunoId);
        $etapaAtual = $progresso['concluidas'] + 1;
        $statusSubmissao = $alunoService->verificarStatusSubmissao($alunoId, $etapaAtual);
        if ($statusSubmissao === 'NECESSITA_REVISAO') {
...
<h1 class="fs-4 fw-semibold text-dark mb-3">Ciclo de Aprendizagem</h1>

<?php
$totalEtapasNav = (int) ($progresso['total_etapas'] ?? 5);
$concluidasNav = (int) ($progresso['concluidas'] ?? 0);
$limiteEtapaNav = min($concluidasNav + 1, $totalEtapasNav);
$etapaCorrenteNav = $concluidasNav >= $totalEtapasNav ? $totalEtapasNav : $concluidasNav + 1;
?>
<nav class="ciclo-sub-nav d-lg-none mb-4" aria-label="Etapas do ciclo">
    <div class="ciclo-sub-nav-inner">
        <?php for ($n = 1; $n <= $totalEtapasNav; $n++):
            $desbloqueadaNav = $n <= $limiteEtapaNav;
            $ativaNav = $desbloqueadaNav && $concluidasNav < $totalEtapasNav && $n === $etapaCorrenteNav;
            $concluidaNav = $n <= $concluidasNav;
            $itemClass = 'ciclo-sub-nav-item';
            if ($ativaNav) {
                $itemClass .= ' is-current';
            } elseif ($concluidaNav) {
                $itemClass .= ' is-done';
            } elseif (!$desbloqueadaNav) {
                $itemClass .= ' is-locked';
            }
        ?>
        <span class="<?= $itemClass ?>" title="Etapa <?= $n ?>">
            <?php if (!$desbloqueadaNav): ?>
                <i class="fas fa-lock" aria-hidden="true"></i>
            <?php elseif ($concluidaNav): ?>
                <i class="fas fa-check" aria-hidden="true"></i>
            <?php else: ?>
                <span class="ciclo-sub-nav-num"><?= $n ?></span>
            <?php endif; ?>
            <span class="ciclo-sub-nav-label">E<?= $n ?></span>
        </span>
        <?php endfor; ?>
    </div>
</nav>
            ...
            <div id="<?= $accId ?>" class="accordion-collapse collapse" data-bs-parent="#historicoEtapas">
                <div class="accordion-body">
                    <?php if ($i === 1): ?>
                        ...
                        <?php
                        $linkHistorico = philoquest_url_segura($dadosAntigos['link_submissao'] ?? null);
                        if ($linkHistorico !== null):
                        ?>
                            <p class="mb-0"><strong>Link de Referência:</strong>
                                <a href="<?= htmlspecialchars($linkHistorico) ?>" target="_blank" rel="noopener noreferrer" class="text-primary">
                                    <?= htmlspecialchars($linkHistorico) ?>
                                </a>
                            </p>
                        <?php endif; ?>
                    <?php elseif ($i === 2): ?>
