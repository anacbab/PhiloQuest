...
/** Mensagem segura para exibir ao utilizador (nunca expõe detalhes de PDO). */
function philoquest_user_error_message(Throwable $e, string $fallback = 'Ocorreu um erro. Tente novamente.'): string
{
...
/**
 * Valida upload de CSV antes de processar.
 *
 * @param array<string, mixed> $file Entrada $_FILES['campo']
 * @return array{ok: true, path: string}|array{ok: false, erro: string}
 */
function philoquest_validar_upload_csv(array $file, int $maxBytes = 2_097_152): array
{
    if (!isset($file['error']) || !is_int($file['error'])) {
        return ['ok' => false, 'erro' => 'Nenhum ficheiro foi enviado.'];
    }

    if ($file['error'] === UPLOAD_ERR_NO_FILE) {
        return ['ok' => false, 'erro' => 'Selecione um ficheiro CSV.'];
    }

    if ($file['error'] !== UPLOAD_ERR_OK) {
        return ['ok' => false, 'erro' => 'Falha no envio do ficheiro. Tente novamente.'];
    }
    ...
    if ($size <= 0) {
        return ['ok' => false, 'erro' => 'O ficheiro está vazio.'];
    }

    if ($size > $maxBytes) {
        $limiteMb = round($maxBytes / 1_048_576, 1);
        return ['ok' => false, 'erro' => "O ficheiro excede o limite de {$limiteMb} MB."];
    }

    $nome = (string) ($file['name'] ?? '');
    $extensao = strtolower(pathinfo($nome, PATHINFO_EXTENSION));
    if ($extensao !== 'csv') {
        return ['ok' => false, 'erro' => 'Apenas ficheiros .csv são permitidos.'];
    }

    $mime = mime_content_type($tmpPath);
    $mimesPermitidos = ['text/plain', 'text/csv', 'application/csv', 'application/vnd.ms-excel'];
    if ($mime !== false && !in_array($mime, $mimesPermitidos, true)) {
        return ['ok' => false, 'erro' => 'O tipo do ficheiro não é um CSV válido.'];
    }
...
/** Aceita apenas URLs http/https; retorna null se inválida ou esquema perigoso (ex.: javascript:). */
function philoquest_url_segura(?string $url): ?string
{
    if ($url === null) {
        return null;
    }
...
/**
 * Processa POST de vinculação de turma (professor). Redireciona se a requisição for tratada.
 */
function philoquest_processar_vinculo_turma_post(\PhiloQuest\Services\ProfessorService $service, int $professorId, string $redirectUrl): void
{
    if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['vincular_turma'])) {
        return;
    }

    if (!philoquest_csrf_verify()) {
        philoquest_csrf_fail_redirect($redirectUrl);
    }

    $codigoTurma = trim($_POST['codigo_turma'] ?? '');
    if ($codigoTurma !== '') {
        try {
            $service->vincularTurma($professorId, $codigoTurma);
            philoquest_flash_set('sucesso', "Turma {$codigoTurma} vinculada com sucesso.");
        } catch (Exception $e) {
            philoquest_flash_set('erro', philoquest_user_error_message($e, 'Não foi possível vincular a turma.'));
        }
    }
...
function philoquest_get_menu_items(string $tipoUsuario): array
{
    $menus = [
        'ADMIN' => [
            ['href' => 'gerenciar_usuarios.php', 'icon' => 'fa-users', 'label' => 'Usuários', 'short' => 'Usuários'],
            ['href' => 'gerenciar_matriculas.php', 'icon' => 'fa-user-graduate', 'label' => 'Alunos', 'short' => 'Alunos'],
            ['href' => 'cadastrar_professor.php', 'icon' => 'fa-chalkboard-teacher', 'label' => 'Professores', 'short' => 'Profs'],
            ['href' => 'dashboard.php', 'icon' => 'fa-home', 'label' => 'Painel inicial', 'short' => 'Painel'],
        ],
        'PROFESSOR' => [
            ['href' => 'gerenciar_turmas.php', 'icon' => 'fa-chalkboard-teacher', 'label' => 'Turmas', 'short' => 'Turmas'],
            ['href' => 'validacao.php', 'icon' => 'fa-check-circle', 'label' => 'Validações', 'short' => 'Validar'],
            ['href' => 'trabalhos.php', 'icon' => 'fa-file-lines', 'label' => 'Trabalhos', 'short' => 'Trabalhos'],
            ['href' => 'missoes_extras.php', 'icon' => 'fa-bolt', 'label' => 'Missão Extra', 'short' => 'Missão'],
            ['href' => 'dashboard.php', 'icon' => 'fa-home', 'label' => 'Painel inicial', 'short' => 'Painel'],
        ],
        'ALUNO' => [
            ['href' => 'ciclo.php', 'icon' => 'fa-sync-alt', 'label' => 'Ciclo de Aprendizagem', 'short' => 'Ciclo'],
            ['href' => 'avaliacoes.php', 'icon' => 'fa-check-circle', 'label' => 'Avaliações', 'short' => 'Avaliar'],
            ['href' => 'missao_extra.php', 'icon' => 'fa-bolt', 'label' => 'Missão Extra', 'short' => 'Missão'],
            ['href' => 'mural.php', 'icon' => 'fa-desktop', 'label' => 'Mural', 'short' => 'Mural'],
            ['href' => 'dashboard.php', 'icon' => 'fa-home', 'label' => 'Painel inicial', 'short' => 'Painel'],
        ],
    ];
