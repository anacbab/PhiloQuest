...
class MissaoExtraService
{
    ...
    public function criarMissao(
        ...
        if ($linkReferencia !== null && $linkReferencia !== '') {
            $linkSeguro = philoquest_url_segura($linkReferencia);
            if ($linkSeguro === null) {
                throw new InvalidArgumentException('Link de referência inválido. Use apenas http:// ou https://.');
            }
            $linkReferencia = $linkSeguro;
        } else {
            $linkReferencia = null;
        }
        ...
        try {
            ...
            throw ExceptionHelper::fromPdo($e, 'Erro ao criar missão.');
        }
    }
        ...
        try {
            $stmt = $this->conexao->prepare(
                "SELECT em.id, em.resposta_texto, em.status, em.feedback_professor, em.data_entrega,
                        em.xp_atribuido, u.nome_completo AS aluno_nome, u.matricula AS aluno_matricula,
                        u.id AS aluno_id
    ...
    public function aprovarEntrega(
        ...
        try {
            ...
            throw ExceptionHelper::fromPdo($e, 'Erro ao aprovar entrega.');
        }
    }
    ...
    public function solicitarRevisaoEntrega(int $entregaId, int $professorId, string $feedback): bool
    {
        ...
        try {
            $stmt = $this->conexao->prepare(
                "UPDATE entregas_missoes
                 SET status = :status, feedback_professor = :feedback,
                     data_avaliacao = NOW(), avaliado_por = :prof_id, xp_atribuido = 0
                 WHERE id = :id"
            );
            $stmt->execute([
                ':status' => StatusEntregaMissao::REVISAR->value,
            ...
            throw ExceptionHelper::fromPdo($e, 'Erro ao solicitar revisão.');
        }
    }
    ...
    public function submeterResposta(int $alunoId, int $missaoId, string $resposta): bool
    {
        ...
        try {
            ...
            throw ExceptionHelper::fromPdo($e, 'Erro ao enviar resposta.');
        }
    }
