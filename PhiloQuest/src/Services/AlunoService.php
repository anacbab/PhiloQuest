...
class AlunoService
{
   ...
   /**
    ...
    public function submeterEtapa(int $alunoId, int $etapaId, string $titulo, string $descricao, string $link): bool {
        ...
        $link = trim($link);
        if ($link !== '') {
            $linkSeguro = philoquest_url_segura($link);
            if ($linkSeguro === null) {
                throw new Exception('Link inválido. Use apenas URLs que comecem com http:// ou https://.');
            }
            $link = $linkSeguro;
        }
        ...
        try {
            ...
            throw ExceptionHelper::fromPdo($e, 'Erro interno ao salvar no banco de dados.');
        }
    }
    ...
    public function calcularNivel(int $xp): string {
        ...
        if ($xp < 3000) return 'Mestre em Filosofia';
        return 'Sábio Supremo';
    }

    /**
     * Ranking da turma por XP acumulado no ciclo (campo experiencia_total).
     *
     * @return list<array{nome_completo: string, experiencia_total: int}>
     */
    public function obterRankingTurma(int $alunoId): array {
        try {
            ...
            $sql = "SELECT nome_completo, COALESCE(experiencia_total, 0) AS experiencia_total
                    FROM usuarios
        ...
        try {
            $sql = "SELECT se.id, se.data_submissao, se.data_validacao, se.status, se.feedback, se.nota,
                    ...
                    WHERE se.aluno_id = :aluno_id
                      AND (
                          se.status != 'AGUARDANDO_VALIDACAO'
                          OR se.data_validacao IS NOT NULL
                      )
                    ORDER BY COALESCE(se.data_validacao, se.data_submissao) DESC";
            ...
            error_log('AlunoService::obterHistoricoAvaliacoes: ' . $e->getMessage());
            return [];
        }
    }
