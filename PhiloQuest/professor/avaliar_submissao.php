...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($feedbackGeral === '') {
        $mensagemErro = 'Escreva um comentário para o aluno antes de enviar.';
    } else {
        try {
            ...
            if ($numeroEtapa === 2 && $statusEnum->obterXP() > 0) {
                $perguntasEtapa2 = Etapa2AvaliacaoHelper::extrairPerguntas((string) ($submissao['descricao_submissao'] ?? ''));
                $totalPerguntas = count($perguntasEtapa2);
                if ($totalPerguntas > 0) {
                    $notasValidas = 0;
                    for ($i = 0; $i < $totalPerguntas; $i++) {
                        if (trim((string) ($notasIndividuais[$i] ?? '')) !== '') {
                            $notasValidas++;
                        }
                    }
                    if ($notasValidas < $totalPerguntas) {
                        throw new \InvalidArgumentException('Avalie todas as perguntas antes de aprovar a Etapa 2.');
                    }
                }
            }
            ...
            $mensagemErro = philoquest_user_error_message($e, 'Erro ao registrar a avaliação.');
        }
    }
}
        ...
        <div class="col-lg-8">
            <div class="card border-0 rounded-4 p-4 mb-4">
                ...
                <div class="bg-light rounded-3 p-3 mb-4 small">
                    ...
                    <div class="text-muted"><?= htmlspecialchars((string) $submissao['aluno_matricula']) ?></div>
                ...
                <?php else: ?>
                    ...
                    <?php
                    $linkSubmissao = philoquest_url_segura((string) ($submissao['link_submissao'] ?? ''));
                    if ($linkSubmissao !== null):
                    ?>
                        <p class="mt-3 mb-0">
                            <strong>Link:</strong>
                            <a href="<?= htmlspecialchars($linkSubmissao) ?>" target="_blank" rel="noopener noreferrer" class="text-primary">
                                <?= htmlspecialchars($linkSubmissao) ?>
                            </a>
                        </p>
                    <?php endif; ?>
                <?php endif; ?>
