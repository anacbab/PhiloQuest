...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($acao === 'aprovar') {
            $statusXp = (string) ($_POST['status_xp'] ?? '');
            ...
            $feedback = trim((string) ($_POST['feedback'] ?? ''));
            // Revisão nunca concede XP — ignora status_xp enviado pelo formulário
...
<div class="card border-0 rounded-4 bg-stat-lilac mb-4">
    <div class="card-body p-4">
        <div class="d-flex align-items-start gap-3 mb-3">
            ...
            <div>
                ...
                <h1 class="h4 fw-semibold text-dark mb-2"><?= htmlspecialchars($missao['titulo']) ?></h1>
                <p class="small text-muted mb-0"><?= nl2br(htmlspecialchars($missao['descricao'])) ?></p>
            </div>
        </div>
        <?php
        $linkRefMissao = philoquest_url_segura($missao['link_referencia'] ?? null);
        if ($linkRefMissao !== null):
        ?>
            <a href="<?= htmlspecialchars($linkRefMissao) ?>" target="_blank" rel="noopener noreferrer"
            ...
            <div class="card-body p-4">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3">
                    <div>
                        ...
                        <p class="small text-muted mb-0"><?= htmlspecialchars($entrega['aluno_matricula']) ?></p>
                ...
                <form method="POST" class="missao-form-avaliacao">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <button type="submit" name="acao_avaliar" value="revisar" class="btn btn-outline-secondary philo-btn-action" formnovalidate
                                onclick="this.form.querySelector('[name=status_xp]').value=''; return confirm('Solicitar revisão? O aluno precisará reenviar a resposta.');">
