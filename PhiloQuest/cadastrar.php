        ...
        if ($tipoUsuario === 'ALUNO') {
            if ($codigoTurma === '') {
                ...
                try {
                    ...
                    $mensagemErroBanco = philoquest_user_error_message($e);
                }
            }
...
?>
        <div class="card-body p-4 p-md-5">
            ...
            <form id="formCadastro" method="POST">
                ...
                <div class="mb-3 text-start<?= $tipoUsuario === 'PROFESSOR' ? ' d-none' : '' ?>" id="grupo_turma">
                    ...
                    <input type="text" class="form-control rounded-3" id="codigo_turma" name="codigo_turma"
                           placeholder="Ex: 103HS2026"<?= $tipoUsuario === 'ALUNO' ? ' required' : '' ?>
...
<script src="<?= htmlspecialchars(philoquest_asset('js/validacao.js')) ?>"></script>
<script>
document.addEventListener('DOMContentLoaded', function() {
    const caixaMensagem = document.querySelector('.auth-alert');
    const grupoTurma = document.getElementById('grupo_turma');
    const inputTurma = document.getElementById('codigo_turma');
    const erroTurma = document.getElementById('erroTurma');

    function atualizarCampoTurma(tipo) {
        if (!grupoTurma || !inputTurma) return;
        if (tipo === 'PROFESSOR') {
            grupoTurma.classList.add('d-none');
            inputTurma.removeAttribute('required');
            inputTurma.value = '';
            inputTurma.classList.remove('is-invalid');
            if (erroTurma) erroTurma.classList.add('d-none');
        } else {
            grupoTurma.classList.remove('d-none');
            inputTurma.setAttribute('required', 'required');
        }
    }
    ...
    document.querySelectorAll('input[name="tipo_usuario"]').forEach(function(botao) {
        botao.addEventListener('change', function() {
            ...
            atualizarCampoTurma(botao.value);
        });
    });

    const selecionado = document.querySelector('input[name="tipo_usuario"]:checked');
    atualizarCampoTurma(selecionado ? selecionado.value : 'ALUNO');
});
</script>
